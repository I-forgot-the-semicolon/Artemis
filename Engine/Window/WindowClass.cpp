...
WindowClass::WindowClass() noexcept
{
    ...
    redirectToConsole();
    std::cout << "Creating actual Window class" << std::endl;
    wndClass.hInstance = this->hInstance;
    wndClass.lpszClassName = GetName();
    wndClass.cbSize = sizeof(wndClass);
    wndClass.style = CS_OWNDC | CS_DBLCLKS; // Testing
}
...
{
    std::cout << "Deleting actualWindow class" << std::endl;
    UnregisterClassA(windowClassName, GetInstance());
}

void WindowClass::redirectToConsole()
{
    AllocConsole();
